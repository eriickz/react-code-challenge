...
const UserItem: React.FC<UserItemProps> = ({ user, deleteUserMutation }) => {
      ...
      <div className="w-full">
        <div className="flex mb-4">
          ...
          <div className="flex flex-col justify-center font-serif ml-2">
            <p className="text-green text-lg font-semibold">{`${user.first_name} ${user.second_name}`}</p>
        ...
        <Button btnType="delete" onClick={() => deleteUserMutation.mutate(user.id)}>
          Delete User
        </Button>
      </div>
    </div>
